...
const emit = defineEmits(['update:url', 'onSuccess', 'deleteUpload', 'uploadurl'])
...
const beforeUpload: UploadProps['beforeUpload'] = async (file) => {
  ...
  if (isTypeOk && isSizeOk) {
    ...
    // uploadData.value.token = data.token
...
const onSuccess: UploadProps['onSuccess'] = (res) => {
  uploadData.value.progress.preview = 'http://thermo-gene-live.img.qiniu.synconize.com/' + res.key
...
<template>
  <div class="upload-container">
    <el-upload :show-file-list="false" :headers="headers" :action="action" :data="uploadData" :name="name"
               :before-upload="beforeUpload" :on-progress="onProgress" :on-success="onSuccess" drag
               class="image-upload">
    ...
    <div v-if="!notip" class="el-upload__tip">
      <div style="display: inline-block;">
        <el-alert :title="`上传图片支持 ${ext.join(' / ')} 格式，且图片大小不超过 ${size}MB，建议图片尺寸为900*383`"
                  type="info" show-icon
                  :closable="false"/>
      </div>
    </div>
